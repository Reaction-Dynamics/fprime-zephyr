...
public:
  ...
  //!
  void
  asyncSend_handler(const FwIndexType portNum, //!< The port number
  ...
  send_handler(const FwIndexType portNum, //!< The port number
               Fw::Buffer &sendBuffer     //!< The buffer to send through UART
  );
...
private:
  void static uartEventCallback(const struct device *dev,
